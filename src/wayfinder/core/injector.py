...
# nspasteboard.org markers honoured by clipboard managers (Maccy, Raycast,
# Paste, Alfred...) and macOS clipboard history: "don't record this".
_MACOS_TRANSIENT_TYPES = ("org.nspasteboard.TransientType", "org.nspasteboard.AutoGeneratedType")
# Restoring more than this is not worth forcing every lazily-provided format
# (Excel/Photoshop can offer dozens) on the paste path.
_MACOS_SNAPSHOT_MAX_BYTES = 64 * 1024 * 1024
# Snapshot sentinel: reading the clipboard would prompt/deny, or it is too big.
_NO_SNAPSHOT = object()


def _macos_pasteboard_readable(pasteboard) -> bool:
    """False when reading would show macOS's paste-permission prompt or is denied."""
    try:
        behavior = int(pasteboard.accessBehavior())
    except Exception:
        return True  # older macOS: no pasteboard privacy
    from AppKit import NSPasteboardAccessBehaviorAlwaysDeny, NSPasteboardAccessBehaviorAsk

    return behavior not in (NSPasteboardAccessBehaviorAsk, NSPasteboardAccessBehaviorAlwaysDeny)
...
def _snapshot_macos_pasteboard():
    ...
    if not _macos_pasteboard_readable(pasteboard):
        return pasteboard, _NO_SNAPSHOT
    items = []
    total = 0
    for item in pasteboard.pasteboardItems() or ():
        ...
        for type_name in item.types() or ():
            ...
            if data is not None:
                raw = bytes(data)
                total += len(raw)
                if total > _MACOS_SNAPSHOT_MAX_BYTES:
                    return pasteboard, _NO_SNAPSHOT
                payload.append((str(type_name), raw))
        items.append(payload)
    return pasteboard, items


def _prepare_macos_pasteboard(pasteboard) -> None:
    """Clear for Aura's own write, kept to this Mac (no Universal Clipboard)."""
    try:
        from AppKit import NSPasteboardContentsCurrentHostOnly
...
def _mark_macos_transient(pasteboard) -> None:
    try:
        from Foundation import NSData

        empty = NSData.data()
        for marker in _MACOS_TRANSIENT_TYPES:
            pasteboard.setData_forType_(empty, marker)
    except Exception:
        pass


def _write_macos_pasteboard_text(pasteboard, text: str) -> int:
    """Put the dictation on the clipboard for the paste only.

    Transient + this-Mac-only: it must not land in clipboard-manager history,
    Spotlight clipboard history, or the user's other devices.
    """
    from AppKit import NSPasteboardTypeString

    _prepare_macos_pasteboard(pasteboard)
    ...
    _mark_macos_transient(pasteboard)
...
def _restore_macos_pasteboard(pasteboard, items) -> bool:
    ...
    if items is _NO_SNAPSHOT:
        return True  # never read (privacy prompt / too big): leave it alone
    restored = []
    ...
    # The user's own content coming back is not a new copy: keep it out of
    # clipboard history and off other devices too.
    if restored:
        empty = NSData.data()
        for marker in _MACOS_TRANSIENT_TYPES:
            restored[0].setData_forType_(empty, marker)
    _prepare_macos_pasteboard(pasteboard)
    if not restored:
        return True
